...
df_intermediate <- read_csv("data/df_tracts_summarized_pa.csv", col_types = cols(.default = "c")) %>% 
...
pa_tracts_centroids <- cbind(pa_tracts, st_coordinates(st_centroid(pa_tracts))) %>% 
...
pa_tracts %>% 
  ggplot() +
  geom_sf() +
  geom_point(data = pa_tracts_centroids, aes(x, y), size = .2)

#pa_tracts_centroids <- pa_tracts_centroids %>% 
#  semi_join(df_intermediate, by = c("GEOID" = "h_tract")) %>% 
#  semi_join(df_intermediate, by = c("GEOID" = "w_tract"))

#df_intermediate <- df_intermediate %>% 
#  semi_join(pa_tracts_centroids, by = c("h_tract" = "GEOID")) %>% 
#  semi_join(pa_tracts_centroids, by = c("w_tract" = "GEOID"))
...
pa_tracts %>% 
  ggplot() +
  geom_sf(aes(fill = commuters_in), color = NA, size = .05) +
  scale_fill_viridis_c()

pa_tracts %>% 
  ggplot() +
  geom_sf(aes(fill = commuters_out), color = NA, size = .05) +
  scale_fill_viridis_c()
...
main_graph <- ggraph(manual_layout) +
                      geom_sf(data = pa_tracts, aes(fill = commuters_out), color = NA) +
...
ggsave("output/images/main_graph_pa.png", main_graph, device = "png",  width = 12, height = 12)
##################################
#to filter on edges
...
ggraph(manual_layout_filtered) +
  geom_sf(data = pa_tracts, aes(fill = commuters_out), size = .01) +
  geom_sf(data = pa_tracts %>%  filter(GEOID == filter_tract), linetype = 2, alpha = 0, size = 2) +
  geom_sf(data = pa_tracts_highlight, color = "red", alpha = 0) +
